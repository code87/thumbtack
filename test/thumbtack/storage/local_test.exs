defmodule Thumbtack.Storage.LocalTest do
  ...
  describe "configuration" do
    test "root_url/0 returns configured root url" do
      assert "http://localhost:4000/uploads" == Thumbtack.Storage.Local.root_url()
    end

    test "storage_path/0 returns configured path" do
      assert "tmp/uploads" == Thumbtack.Storage.Local.storage_path()
    end
  end
  ...
  describe "put(src_path, dest_path)" do
    ...
    test "copies existing file and returns URL", %{src_path: src_path} do
      assert {:ok, "http://localhost:4000/uploads/dummy/file.txt"} = Thumbtack.Storage.Local.put(src_path, "/dummy/file.txt")
    ...
    test "returns error tuple if source file does not exist" do
      ...
      assert {:error, :enoent} = Thumbtack.Storage.Local.put(src_path, "/dummy/file.txt")
    end

    test "returns error tuple if src_path is not a file path" do
      src_path = System.tmp_dir()
      assert {:error, _reason} = Thumbtack.Storage.Local.put(src_path, "/dummy/file.txt")
    end
  end
  ...
  describe "delete(path)" do
    test "deletes existing file and returns URL" do
      ...
      assert {:ok, "http://localhost:4000/uploads/dummy/file.txt"} = Thumbtack.Storage.Local.delete("/dummy/file.txt")
    ...
    test "returns error tuple if file does not exist" do
      ...
      assert {:error, :enoent} = Thumbtack.Storage.Local.delete("dummy/unknown.txt")
    end
  end
end
